...
public class ArrayLogic
{
	...
	public void printArrays(int arr[],int num)
	{
		int count=num-1;
		int value=0;
		for(int i=0;i<num;i++)
	...
	public void closestSumArray(int arr[],int num,int closest)
	{
		int max=0;
		int currentMax=0;
		int value1=0;
		...
		for(int i=0;i<num;i++)
		{
			...
			{
				if(arr[i]+arr[j]<=closest)
				{
					max=Math.max(0,arr[i]+arr[j]);
					value1=arr[i];
					value2=arr[j];
				}
			}
	...
	public void sumPairArray(int arr[],int num,int sum)
	{
		int max=0;
		int currentMax=0;
		int value1=0;
	...
	public void multiplyArray(int arr[],int num)
	{
		int count=0;
		int currentMax=0;
		int multi=1;
		for(int i=0;i<num;i++)
		{
			for(int j=i+1;j<num;j++)
			{
				...
				{
					currentMax=Math.max(currentMax,arr[i]*arr[j]*arr[k]);
					multi=Math.max(multi,currentMax);
				}
			}
		}
